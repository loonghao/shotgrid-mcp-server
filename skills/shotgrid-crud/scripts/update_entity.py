...
def main():
    parser = argparse.ArgumentParser(
        description="Update an existing entity in ShotGrid"
    )
    parser.add_argument(
        "--entity_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=json.loads,
        required=True,
        help="Fields to update as JSON object (key-value pairs)",
    )
    args = parser.parse_args()

    try:
        sg = get_current_shotgrid_connection()
        result = update_sg_entity(sg, args.entity_type, args.entity_id, args.data)
