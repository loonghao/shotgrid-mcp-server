...
ShotGrid MCP Server follows the standard MCP configuration format. Since it's a Python-based service, it uses `uvx` as the command to launch the server. Here's how to configure it in your MCP client configuration file (e.g., `claude_desktop_config.json`):
...
This configuration tells the MCP client how to launch and connect to the ShotGrid MCP Server. The server will be available to AI assistants as the "shotgrid" tool provider.
