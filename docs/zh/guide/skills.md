...
不是合法 UTF-8 的 `text/*` 文件（latin-1、UTF-16 等）没有无损的文本形式，
因此会以 base64 `blob` 下发，而不是让读取失败。`mimeType` 仍表示文件**本身**
是什么（`text/markdown`），`blob` 才是传输编码的信号。请按字节解码，不要按
UTF-8 文本解码——`size` 与 `digest` 对应的正是这些字节。
